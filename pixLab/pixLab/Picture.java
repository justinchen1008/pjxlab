...
public class Picture extends SimplePicture 
{
 ...
 public Picture stairStep(int shiftCount, int steps){
  ...
  /** distorts the picture and returns a smooth version of the distortion
  ...
  public Picture liquify(int maxHeight){
	...
	for(int row = 0; row < pixels.length; row++){
		for(int col = 0; col < pixels[row].length; col++){
			...
			resultPixels[row][(col + rightShift) % pixels[row].length].setGreen(pixels[row][col].getGreen());
			resultPixels[row][(col + rightShift) % pixels[row].length].setBlue(pixels[row][col].getBlue());
			resultPixels[row][(col + rightShift) % pixels[row].length].setRed(pixels[row][col].getRed());
		}
	}
	return result;
  }
  ...
  public Picture wavy(int amplitude){
	...
	for(int row = 0; row < pixels.length; row++){
		for(int col = 0; col < pixels[row].length; col++){
			double input = 2.0 * Math.PI * frequency * (double)(row);
			int rightShift = (int)(amplitude * Math.sin(Math.toRadians(input % 180)));
